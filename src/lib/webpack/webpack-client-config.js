			...
			loaders: [
				{
					...
					options: {
						name(filename) {
							...
							return isRoute ? 'route-' + relative.replace(/(^\/(routes|components|translate|utils|constants|resources\/(routes|async))\/|(\/index)?\.js$)/g, '') : false;
						},
						formatName(filename) {
							let relative = filename.replace(source('.'), '');
							// strip out context dir & any file/ext suffix
							return relative.replace(/(^\/(routes|components|translate|utils|constants|resources\/(routes|async))\/|(\/index)?\.js$)/g, '');
						}
					}
				}
			]
